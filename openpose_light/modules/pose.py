...
from openpose_light.modules.keypoints import BODY_PARTS_KPT_IDS, BODY_PARTS_PAF_IDS
from openpose_light.modules.one_euro_filter import OneEuroFilter


class Pose:
    num_kpts = 18
    head_kpt_indexs = [0, 1, 14, 15, 16, 17]
    kpt_names = ['nose', 'neck',
    ...
    def __init__(self, keypoints, confidence):
        ...
        self.head_bbox = Pose.get_head_bbox(self.keypoints)
        self.id = None
    ...
    def get_head_bbox(keypoints):
        found_keypoints = np.zeros((np.count_nonzero(keypoints[Pose.head_kpt_indexs, 0] != -1), 2), dtype=np.int32)
        found_kpt_id = 0
        for kpt_id in Pose.head_kpt_indexs:
            if keypoints[kpt_id, 0] == -1:
                continue
            found_keypoints[found_kpt_id] = keypoints[kpt_id]
            found_kpt_id += 1
